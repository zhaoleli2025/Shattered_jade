...
function mkUnit(t, q, r, su = {}) {
  const u = Object.assign(JSON.parse(JSON.stringify(t)), {
    q, r, hp: t.hpMax,
    morale: "Steady", fledRounds: 0, bleed: 0, alive: true,
    ap: 0, attacksLeftHint: 0,
  });
  u.wpn.duraNow = u.wpn.dura;            // the edge is fresh — for now
  if (u.wpn2) u.wpn2.duraNow = u.wpn2.dura;
  // 品阶: scenario unit entry overrides the template, default 凡品.
  // All four slots resolve AND validate eagerly — an invalid wpn2_q fails loud
  // even when the unit carries no second weapon (parity with sim/state.py).
  const qids = {};
  for (const slot of ["wpn_q", "wpn2_q", "armor_q", "helmet_q"]) {
    qids[slot] = su[slot] ?? t[slot] ?? "fan";
    qualityGrade(qids[slot]);
  }
  gradeWeapon(u.wpn, qids.wpn_q);
  if (u.wpn2) gradeWeapon(u.wpn2, qids.wpn2_q);
  const body = gradeArmor(ARMOR[u.armor], qids.armor_q);
  const helm = gradeArmor(ARMOR[u.helmet], qids.helmet_q);
  // 坠气: everything strapped on drags on the wind — body, helm, weapon, and the
  // sheathed sidearm all tax max Breath. 换械 swaps weapons without recomputing.
  u.carryTax = body.br_tax + helm.br_tax + u.wpn.br_tax + (u.wpn2 ? u.wpn2.br_tax : 0);
  u.breathMax = u.breathBase - u.carryTax;
  u.breath = u.breathMax;
  u.armorB = body.protect; u.armorH = helm.protect;
  u.armorName = body.label; u.helmName = helm.label;
  u.armorQ = body.quality; u.helmQ = helm.quality;
...
function rosterTemplates() {
  return [
    P("wang", "王铁枪", "枪", { hpMax: 55, skill: 62, def: 10, shield: 0, resolve: 48,
      ...
      wpn: { kind: "melee", label: "长枪", dura: 64, hands: 2, reach: 2, acc: 20, dmin: 22, dmax: 32, armorEff: 0.9, pierce: 0.3, ap: 6, br: 15, br_tax: 4, special: { type: "spearwall", label: "枪林", ap: 3, br: 18 } },
      wpn2: { kind: "melee", label: "腰刀", dura: 56, hands: 1, acc: 10, dmin: 18, dmax: 26, armorEff: 1.0, pierce: 0.3, ap: 4, br: 12, br_tax: 2, bleed: true, special: { type: "decap", label: "斩首", ap: 5, br: 15, dmgMult: 1.3 } } }),
    P("liu", "刘三刀", "刀", { hpMax: 60, skill: 60, def: 6, shield: 15, resolve: 45,
      initBase: 104, breathBase: 87, armor: "pijia", helmet: "pikui", wpn_q: "jing",
      wpn: { kind: "melee", label: "腰刀", dura: 56, hands: 1, acc: 10, dmin: 18, dmax: 26, armorEff: 1.0, pierce: 0.3, ap: 4, br: 12, br_tax: 2, bleed: true, special: { type: "decap", label: "斩首", ap: 5, br: 15, dmgMult: 1.3 } } }),
    P("shi", "石敢当", "锤", { hpMax: 65, skill: 58, def: 5, shield: 0, resolve: 50,
      initBase: 90, breathBase: 93, armor: "tiejia", helmet: "tiekui",
      wpn: { kind: "melee", label: "大锤", dura: 72, hands: 2, acc: 5, dmin: 28, dmax: 40, armorEff: 2.0, pierce: 0.2, ap: 6, br: 18, br_tax: 6, breathDrain: 20, special: { type: "demolish", label: "碎甲", ap: 6, br: 20, armorMult: 3.0 } } }),
    P("yan", "燕小乙", "弓", { hpMax: 45, skill: 56, def: 8, shield: 0, resolve: 42,
      initBase: 112, breathBase: 94, armor: "bujia", helmet: "bumao", wpn_q: "liang",
      wpn: { kind: "ranged", label: "猎弓", dura: 48, hands: 2, acc: 10, dmin: 14, dmax: 24, armorEff: 0.6, pierce: 0.35, ap: 4, br: 8, br_tax: 2, range: 7, falloff: 3, special: { type: "aimed", label: "瞄准", ap: 6, br: 12, accBonus: 10, falloff: 2 } },
      wpn2: { kind: "melee", label: "匕首", dura: 40, hands: 1, acc: -15, dmin: 14, dmax: 22, armorEff: 0.0, pierce: 1.0, ap: 4, br: 10, br_tax: 1, noHead: true } }),
    E("duyan", "独眼龙", "刀", { hpMax: 50, skill: 52, def: 4, shield: 0, resolve: 38,
      initBase: 100, breathBase: 89, armor: "bujia", helmet: "bumao",
      wpn: { kind: "melee", label: "砍刀", dura: 56, hands: 1, acc: 10, dmin: 17, dmax: 26, armorEff: 1.0, pierce: 0.3, ap: 4, br: 12, br_tax: 2, bleed: true, special: { type: "decap", label: "斩首", ap: 5, br: 15, dmgMult: 1.3 } } }),
    ...
    E("xiaohu", "笑面虎", "斧", { hpMax: 55, skill: 54, def: 4, shield: 0, resolve: 40,
      ...
      wpn: { kind: "melee", label: "大斧", dura: 64, hands: 2, acc: 5, dmin: 30, dmax: 44, armorEff: 1.3, pierce: 0.25, ap: 6, br: 16, br_tax: 5, chop: true, special: { type: "sweep", label: "横扫", ap: 6, br: 20, acc: -10 } } }),
    E("yemao", "夜猫子", "弓", { hpMax: 42, skill: 50, def: 6, shield: 0, resolve: 36,
      initBase: 108, breathBase: 91, armor: "bujia", helmet: "none_h",
      wpn: { kind: "ranged", label: "猎弓", dura: 48, hands: 2, acc: 10, dmin: 14, dmax: 24, armorEff: 0.6, pierce: 0.35, ap: 4, br: 8, br_tax: 2, range: 7, falloff: 3, special: { type: "aimed", label: "瞄准", ap: 6, br: 12, accBonus: 10, falloff: 2 } },
      wpn2: { kind: "melee", label: "短刀", dura: 48, hands: 1, acc: 0, dmin: 14, dmax: 22, armorEff: 0.8, pierce: 0.3, ap: 4, br: 10, br_tax: 1 } }),
    E("diao", "坐山雕", "首", { hpMax: 70, skill: 62, def: 12, shield: 15, resolve: 55,
      ...
      wpn: { kind: "melee", label: "九环刀", dura: 64, hands: 1, acc: 10, dmin: 24, dmax: 34, armorEff: 1.0, pierce: 0.3, ap: 4, br: 12, br_tax: 3, bleed: true, special: { type: "decap", label: "斩首", ap: 5, br: 15, dmgMult: 1.3 } } }),
    ...
    P("chen", "陈短矛", "矛", { hpMax: 58, skill: 58, def: 8, shield: 15, resolve: 46,
      ...
      wpn: { kind: "melee", label: "短矛", dura: 56, hands: 1, acc: 15, dmin: 16, dmax: 24, armorEff: 0.9, pierce: 0.25, ap: 4, br: 11, br_tax: 2, special: { type: "spearwall", label: "枪阵", ap: 3, br: 16 } } }),
    P("he", "何九鞭", "鞭", { hpMax: 52, skill: 57, def: 7, shield: 0, resolve: 44,
      initBase: 106, breathBase: 90, armor: "bujia", helmet: "pikui",
      wpn: { kind: "melee", label: "九节鞭", dura: 56, hands: 1, acc: 5, dmin: 18, dmax: 28, armorEff: 0.9, pierce: 0.3, ap: 4, br: 13, br_tax: 3, ignoreShield: true, headBonus: 10, special: { type: "headhunt", label: "兜头", ap: 5, br: 16, headMult: 2.0, acc: -15, missBr: 15 } } }),
    P("lu", "鲁大弩", "弩", { hpMax: 48, skill: 54, def: 6, shield: 0, resolve: 45,
      initBase: 95, breathBase: 89, armor: "pijia", helmet: "bumao",
      wpn: { kind: "ranged", label: "弩", dura: 48, hands: 2, acc: 15, dmin: 22, dmax: 32, armorEff: 0.8, pierce: 0.5, ap: 4, br: 10, br_tax: 4, range: 6, falloff: 2, special: { type: "aimed", label: "瞄准", ap: 6, br: 14, accBonus: 10, falloff: 1 } },
      wpn2: { kind: "melee", label: "短刀", dura: 48, hands: 1, acc: 0, dmin: 14, dmax: 22, armorEff: 0.8, pierce: 0.3, ap: 4, br: 10, br_tax: 1 } }),
    P("zhou", "周大刀", "关", { hpMax: 60, skill: 58, def: 6, shield: 0, resolve: 47,
      initBase: 93, breathBase: 90, armor: "pijia", helmet: "pikui",
      wpn: { kind: "melee", label: "大关刀", dura: 64, hands: 2, acc: 8, dmin: 28, dmax: 42, armorEff: 1.2, pierce: 0.3, ap: 6, br: 17, br_tax: 6, chop: true, bleed: true, special: { type: "sweep", label: "横扫", ap: 6, br: 20, acc: -10 } } }),
    // ---- the militia line (会战 roster): hired 乡勇 — the player's rank-and-file ----
    P("xya", "乡勇·甲", "勇", { hpMax: 42, skill: 47, def: 4, shield: 0, resolve: 34,
      initBase: 99, breathBase: 86, armor: "bujia", helmet: "bumao",
      wpn: { kind: "melee", label: "长枪", dura: 64, hands: 2, reach: 2, acc: 20, dmin: 22, dmax: 32, armorEff: 0.9, pierce: 0.3, ap: 6, br: 15, br_tax: 4, special: { type: "spearwall", label: "枪林", ap: 3, br: 18 } } }),
    P("xyb", "乡勇·乙", "勇", { hpMax: 42, skill: 46, def: 4, shield: 0, resolve: 33,
      initBase: 98, breathBase: 86, armor: "bujia", helmet: "bumao",
      wpn: { kind: "melee", label: "长枪", dura: 64, hands: 2, reach: 2, acc: 20, dmin: 22, dmax: 32, armorEff: 0.9, pierce: 0.3, ap: 6, br: 15, br_tax: 4, special: { type: "spearwall", label: "枪林", ap: 3, br: 18 } } }),
    P("xyc", "乡勇·丙", "勇", { hpMax: 44, skill: 46, def: 4, shield: 10, resolve: 34,
      initBase: 97, breathBase: 85, armor: "bujia", helmet: "bumao",
      wpn: { kind: "melee", label: "短矛", dura: 56, hands: 1, acc: 15, dmin: 16, dmax: 24, armorEff: 0.9, pierce: 0.25, ap: 4, br: 11, br_tax: 2, special: { type: "spearwall", label: "枪阵", ap: 3, br: 16 } } }),
    P("xyd", "乡勇·丁", "勇", { hpMax: 44, skill: 45, def: 4, shield: 10, resolve: 33,
      initBase: 96, breathBase: 85, armor: "bujia", helmet: "bumao",
      wpn: { kind: "melee", label: "短矛", dura: 56, hands: 1, acc: 15, dmin: 16, dmax: 24, armorEff: 0.9, pierce: 0.25, ap: 4, br: 11, br_tax: 2, special: { type: "spearwall", label: "枪阵", ap: 3, br: 16 } } }),
    E("shemao", "蛇矛子", "矛", { hpMax: 52, skill: 55, def: 6, shield: 0, resolve: 40,
      initBase: 96, breathBase: 89, armor: "pijia", helmet: "bumao",
      wpn: { kind: "melee", label: "长枪", dura: 64, hands: 2, reach: 2, acc: 20, dmin: 22, dmax: 32, armorEff: 0.9, pierce: 0.3, ap: 6, br: 15, br_tax: 4, special: { type: "spearwall", label: "枪林", ap: 3, br: 18 } } }),
    E("zuanshan", "钻山豹", "戟", { hpMax: 54, skill: 55, def: 6, shield: 0, resolve: 41,
      initBase: 97, breathBase: 88, armor: "pijia", helmet: "bumao",
      wpn: { kind: "melee", label: "戟", dura: 64, hands: 2, reach: 2, acc: 12, dmin: 22, dmax: 32, armorEff: 1.0, pierce: 0.35, ap: 6, br: 15, br_tax: 5, special: { type: "sweep", label: "横扫", ap: 6, br: 20, acc: -10 } } }),
    E("lla", "喽啰·甲", "卒", { hpMax: 40, skill: 46, def: 3, shield: 0, resolve: 30,
      initBase: 102, breathBase: 85, armor: "bujia", helmet: "none_h",
      wpn: { kind: "melee", label: "短刀", dura: 48, hands: 1, acc: 0, dmin: 14, dmax: 22, armorEff: 0.8, pierce: 0.3, ap: 4, br: 10, br_tax: 1 } }),
    ...
    E("guoshanfeng", "过山风", "首", { hpMax: 62, skill: 58, def: 9, shield: 15, resolve: 50,
      ...
      wpn: { kind: "melee", label: "九环刀", dura: 64, hands: 1, acc: 10, dmin: 24, dmax: 34, armorEff: 1.0, pierce: 0.3, ap: 4, br: 12, br_tax: 3, bleed: true, special: { type: "decap", label: "斩首", ap: 5, br: 15, dmgMult: 1.3 } } }),
    // ---- the Khitan column (血战 roster): 皮室 iron guard + Han auxiliaries ----
    // Cavalry is M4; this is the dismounted 打草谷 column — 骨朵 mace ≈ 大锤.
    E("xiangwen", "耶律详稳", "辽", { hpMax: 68, skill: 63, def: 12, shield: 15, resolve: 58,
      initBase: 95, breathBase: 92, armor: "tiejia", helmet: "tiekui", leader: true,
      wpn: { kind: "melee", label: "九环刀", dura: 64, hands: 1, acc: 10, dmin: 24, dmax: 34, armorEff: 1.0, pierce: 0.3, ap: 4, br: 12, br_tax: 3, bleed: true, special: { type: "decap", label: "斩首", ap: 5, br: 15, dmgMult: 1.3 } } }),
    E("psa", "皮室·甲", "辽", { hpMax: 60, skill: 59, def: 7, shield: 0, resolve: 52,
      initBase: 94, breathBase: 92, armor: "tiejia", helmet: "tiekui",
      wpn: { kind: "melee", label: "长枪", dura: 64, hands: 2, reach: 2, acc: 20, dmin: 22, dmax: 32, armorEff: 0.9, pierce: 0.3, ap: 6, br: 15, br_tax: 4, special: { type: "spearwall", label: "枪林", ap: 3, br: 18 } } }),
    E("psb", "皮室·乙", "辽", { hpMax: 60, skill: 58, def: 7, shield: 0, resolve: 51,
      initBase: 93, breathBase: 92, armor: "tiejia", helmet: "tiekui",
      wpn: { kind: "melee", label: "长枪", dura: 64, hands: 2, reach: 2, acc: 20, dmin: 22, dmax: 32, armorEff: 0.9, pierce: 0.3, ap: 6, br: 15, br_tax: 4, special: { type: "spearwall", label: "枪林", ap: 3, br: 18 } } }),
    E("psc", "皮室·丙", "辽", { hpMax: 62, skill: 58, def: 7, shield: 0, resolve: 52,
      initBase: 92, breathBase: 91, armor: "tiejia", helmet: "tiekui",
      wpn: { kind: "melee", label: "大斧", dura: 64, hands: 2, acc: 5, dmin: 30, dmax: 44, armorEff: 1.3, pierce: 0.25, ap: 6, br: 16, br_tax: 5, chop: true, special: { type: "sweep", label: "横扫", ap: 6, br: 20, acc: -10 } } }),
    E("psd", "皮室·丁", "辽", { hpMax: 62, skill: 57, def: 7, shield: 0, resolve: 51,
      initBase: 91, breathBase: 93, armor: "tiejia", helmet: "tiekui",
      wpn: { kind: "melee", label: "大锤", dura: 72, hands: 2, acc: 5, dmin: 28, dmax: 40, armorEff: 2.0, pierce: 0.2, ap: 6, br: 18, br_tax: 6, breathDrain: 20, special: { type: "demolish", label: "碎甲", ap: 6, br: 20, armorMult: 3.0 } } }),
    E("sda", "射雕手·甲", "弓", { hpMax: 48, skill: 57, def: 6, shield: 0, resolve: 44,
      initBase: 110, breathBase: 92, armor: "pijia", helmet: "bumao",
      wpn: { kind: "ranged", label: "猎弓", dura: 48, hands: 2, acc: 10, dmin: 14, dmax: 24, armorEff: 0.6, pierce: 0.35, ap: 4, br: 8, br_tax: 2, range: 7, falloff: 3, special: { type: "aimed", label: "瞄准", ap: 6, br: 12, accBonus: 10, falloff: 2 } },
      wpn2: { kind: "melee", label: "短刀", dura: 48, hands: 1, acc: 0, dmin: 14, dmax: 22, armorEff: 0.8, pierce: 0.3, ap: 4, br: 10, br_tax: 1 } }),
    E("sdb", "射雕手·乙", "弓", { hpMax: 48, skill: 56, def: 6, shield: 0, resolve: 43,
      initBase: 109, breathBase: 92, armor: "pijia", helmet: "bumao",
      wpn: { kind: "ranged", label: "猎弓", dura: 48, hands: 2, acc: 10, dmin: 14, dmax: 24, armorEff: 0.6, pierce: 0.35, ap: 4, br: 8, br_tax: 2, range: 7, falloff: 3, special: { type: "aimed", label: "瞄准", ap: 6, br: 12, accBonus: 10, falloff: 2 } },
      wpn2: { kind: "melee", label: "短刀", dura: 48, hands: 1, acc: 0, dmin: 14, dmax: 22, armorEff: 0.8, pierce: 0.3, ap: 4, br: 10, br_tax: 1 } }),
    E("hqa", "汉军枪手·甲", "枪", { hpMax: 50, skill: 53, def: 5, shield: 0, resolve: 42,
      initBase: 98, breathBase: 88, armor: "pijia", helmet: "pikui",
      wpn: { kind: "melee", label: "长枪", dura: 64, hands: 2, reach: 2, acc: 20, dmin: 22, dmax: 32, armorEff: 0.9, pierce: 0.3, ap: 6, br: 15, br_tax: 4, special: { type: "spearwall", label: "枪林", ap: 3, br: 18 } } }),
    E("hqb", "汉军枪手·乙", "枪", { hpMax: 50, skill: 52, def: 5, shield: 0, resolve: 41,
      initBase: 97, breathBase: 88, armor: "pijia", helmet: "pikui",
      wpn: { kind: "melee", label: "长枪", dura: 64, hands: 2, reach: 2, acc: 20, dmin: 22, dmax: 32, armorEff: 0.9, pierce: 0.3, ap: 6, br: 15, br_tax: 4, special: { type: "spearwall", label: "枪林", ap: 3, br: 18 } } }),
    E("hqc", "汉军枪手·丙", "枪", { hpMax: 52, skill: 53, def: 5, shield: 12, resolve: 42,
      initBase: 96, breathBase: 87, armor: "pijia", helmet: "pikui",
      wpn: { kind: "melee", label: "短矛", dura: 56, hands: 1, acc: 15, dmin: 16, dmax: 24, armorEff: 0.9, pierce: 0.25, ap: 4, br: 11, br_tax: 2, special: { type: "spearwall", label: "枪阵", ap: 3, br: 16 } } }),
    E("hna", "汉军弩手·甲", "弩", { hpMax: 46, skill: 53, def: 5, shield: 0, resolve: 41,
      initBase: 99, breathBase: 89, armor: "pijia", helmet: "bumao",
      wpn: { kind: "ranged", label: "弩", dura: 48, hands: 2, acc: 15, dmin: 22, dmax: 32, armorEff: 0.8, pierce: 0.5, ap: 4, br: 10, br_tax: 4, range: 6, falloff: 2, special: { type: "aimed", label: "瞄准", ap: 6, br: 14, accBonus: 10, falloff: 1 } },
      wpn2: { kind: "melee", label: "短刀", dura: 48, hands: 1, acc: 0, dmin: 14, dmax: 22, armorEff: 0.8, pierce: 0.3, ap: 4, br: 10, br_tax: 1 } }),
    E("hnb", "汉军弩手·乙", "弩", { hpMax: 46, skill: 52, def: 5, shield: 0, resolve: 40,
      initBase: 98, breathBase: 89, armor: "pijia", helmet: "bumao",
      wpn: { kind: "ranged", label: "弩", dura: 48, hands: 2, acc: 15, dmin: 22, dmax: 32, armorEff: 0.8, pierce: 0.5, ap: 4, br: 10, br_tax: 4, range: 6, falloff: 2, special: { type: "aimed", label: "瞄准", ap: 6, br: 14, accBonus: 10, falloff: 1 } },
      wpn2: { kind: "melee", label: "短刀", dura: 48, hands: 1, acc: 0, dmin: 14, dmax: 22, armorEff: 0.8, pierce: 0.3, ap: 4, br: 10, br_tax: 1 } }),
  ];
}
...
/* armor tiers: protection is an ablative pool, 坠气 (br_tax) drags on max Breath */
const ARMOR = {
  none_b: { label: "无甲", slot: "body", protect: 0, br_tax: 0 },
  bujia:  { label: "布甲", slot: "body", protect: 25, br_tax: 3 },
  pijia:  { label: "皮甲", slot: "body", protect: 60, br_tax: 8 },
  tiejia: { label: "铁甲", slot: "body", protect: 110, br_tax: 16 },
  none_h: { label: "无盔", slot: "head", protect: 0, br_tax: 0 },
  bumao:  { label: "布帽", slot: "head", protect: 15, br_tax: 1 },
  pikui:  { label: "皮盔", slot: "head", protect: 40, br_tax: 3 },
  tiekui: { label: "铁盔", slot: "head", protect: 80, br_tax: 7 },
};

/* 品阶 quality grades — quality buys numbers, family buys verbs:
   行动力/每击气力/射程/特技 are quality-immune; graded gear is finer-made, so its
   坠气 br_tax lightens (DESIGN canonical table, mirror of sim/data.py) */
const QUALITY = {
  fan:   { label: "凡品", color: "#9b9b9b", rank: 0, dmg: 1.00, acc: 0,  protect: 1.00, br_tax: 1.00 },
  liang: { label: "良品", color: "#3f8f4a", rank: 1, dmg: 1.10, acc: 2,  protect: 1.15, br_tax: 1.00 },
  jing:  { label: "精品", color: "#3873b8", rank: 2, dmg: 1.20, acc: 4,  protect: 1.30, br_tax: 0.95 },
  zhen:  { label: "珍品", color: "#8a4bb0", rank: 3, dmg: 1.35, acc: 7,  protect: 1.50, br_tax: 0.90 },
  shen:  { label: "神品", color: "#d2691e", rank: 4, dmg: 1.50, acc: 10, protect: 1.75, br_tax: 0.85 },
};
function qualityGrade(qid) {
  const g = QUALITY[qid];
  if (!g) throw new Error(`未知品阶 "${qid}"（可用：${Object.keys(QUALITY).join("/")}）`);
  return g;
}
/* round with a float-dust snap: stat×multiplier is always an exact multiple of 0.01 in
   decimal, but IEEE gives e.g. 110*1.15 = 126.4999…99 — canon says 126.5 → 127. The 1e-9
   epsilon can never flip a non-tie (nearest non-.5 decimal is .49, 0.01 away). */
const qRound = (x) => Math.round(x + 1e-9);
function gradeWeapon(w, qid) { // mutates the unit's deep copy, never a base table
  const g = qualityGrade(qid);
  w.dmin = qRound(w.dmin * g.dmg);
  w.dmax = qRound(w.dmax * g.dmg);
  w.acc += g.acc;
  w.br_tax = qRound(w.br_tax * g.br_tax);
  if (qid !== "fan") w.label = g.label + "·" + w.label;
  w.quality = qid;
}
function gradeArmor(base, qid) { // returns an applied copy of the ARMOR entry
  const g = qualityGrade(qid);
  const a = Object.assign({}, base, { quality: qid });
  a.protect = qRound(a.protect * g.protect);
  a.br_tax = qRound(a.br_tax * g.br_tax);
  if (qid !== "fan") a.label = g.label + "·" + a.label;
  return a;
}
/* non-凡品 labels render in their grade's color */
const qSpan = (label, qid) => qid && qid !== "fan"
  ? `<span style="color:${QUALITY[qid].color}">${label}</span>` : label;

const VERB = { 长枪: "刺击", 腰刀: "劈砍", 砍刀: "劈砍", 九环刀: "劈砍", 大锤: "锤击", 大斧: "重劈", 戟: "戳刺", 大关刀: "重劈", 猎弓: "放箭", 匕首: "突刺", 短刀: "刺击", 短矛: "刺击", 九节鞭: "抽击", 弩: "发弩" };
...
function specialOpts(sp) {
  switch (sp.type) {
    ...
    case "demolish": return { demolish: true, armorMult: sp.armorMult, tag: sp.label };
    case "aimed": return { accMod: sp.accBonus, falloff: sp.falloff, tag: sp.label };
    case "sweep": return { accMod: sp.acc, tag: sp.label };
    case "headhunt": return { forceHead: true, headMult: sp.headMult, accMod: sp.acc, missBr: sp.missBr, tag: sp.label };
  }
  return {};
}
...
function applyHit(atk, def, isFree, opts = {}) {
  ...
  if ((atk.wpn.duraNow ?? 1) <= 0) dmg = Math.round(dmg * 0.5); // 卷刃 bites half
  const part = head ? "armorH" : "armorB";
  const armorBefore = def[part];
  let armorDmg, hpDmg, mult = 1;
  if (opts.demolish) { // 碎甲: armor ×mult, then blunt trauma through what's left
    armorDmg = Math.min(armorBefore, Math.round(dmg * opts.armorMult));
    // no head multiplier and no overflow — the ×mult IS the verb
    hpDmg = Math.max(0, Math.round(dmg * atk.wpn.pierce - 0.1 * (armorBefore - armorDmg)));
  } else {
    ...
    if (head) { mult = opts.headMult || (atk.wpn.chop ? 2.25 : 1.5); }
  ...
  // BB durability: steel dulls on steel — melee on a part holding ≥50 armor
  if (atk.wpn.kind === "melee" && armorBefore >= 50 && atk.wpn.duraNow != null) {
    const before = atk.wpn.duraNow;
    atk.wpn.duraNow = Math.max(0, before - 3);
    if (before > 0 && atk.wpn.duraNow === 0)
      log(`${atk.name}的${atk.wpn.label}砍崩了口——<b>卷刃</b>（伤害减半，须修缮）`, "sys");
  }
...
function shotWear(u) {
  if (u.wpn.kind === "ranged" && u.wpn.duraNow != null) {
    const before = u.wpn.duraNow;
    u.wpn.duraNow = Math.max(0, before - 2);
    if (before > 0 && u.wpn.duraNow === 0)
      log(`${u.name}的${u.wpn.label}弦疲弓乏——<b>卷刃</b>（伤害减半，须修缮）`, "sys");
  }
}
...
function moraleCheck(u, mod, reason) {
  ...
  const roll = d100();
  // DESIGN §3.5: the log shows EVERY modifier, not just the combined target
  const parts = `胆识${u.resolve}${adj ? ` +同伴${adj}×3` : ""}${mod ? (mod > 0 ? " +" : " −") + Math.abs(mod) : ""} = ${target}`;
  if (roll <= target) {
    log(`${u.name} 胆识考验（${reason}）：d100=${roll} ≤ ${parts} → 稳住`, "sys");
  } else {
    u.morale = u.morale === "Steady" ? "Wavering" : "Fleeing";
    log(`${u.name} 胆识考验（${reason}）：d100=${roll} > ${parts} → <b>${u.morale === "Wavering" ? "动摇" : "溃逃！"}</b>`, "sys");
...
async function doAttack(u, t, useSpecial) {
  ...
  if (useSpecial && sp.type === "sweep") {
    ...
    log(`${u.name} 抡圆了${u.wpn.label.split("·").pop()}——<b>横扫</b>！`, u.side === "player" ? "b" : "r");
    for (const v of victims) { if (v.alive) applyHit(u, v, false, opts); }
  } else {
    const landed = applyHit(u, t, false, opts);
    // 兜头 gamble: the overswing — a whiff drains extra Breath
    if (!landed && opts.missBr) {
      u.breath = Math.max(0, u.breath - opts.missBr);
      log(`${u.name} 一鞭抡空，泄了大气——气力额外 −${opts.missBr}`, "sys");
      float(u, `气−${opts.missBr}`, "#6f6450");
    }
  }
  ...
  for (let safety = 0; safety < 8 && u.alive && !gameOver; safety++) {
    ...
    if (tgts.length) {
      ...
      if (sp && canSpecial(u)) {
        ...
        } else if (sp.type === "demolish") {
          const tough = tgts.find(x => x.armorB >= 50);
          if (tough) { await doAttack(u, tough, true); continue; }
        } else if (sp.type === "headhunt") {
          // gamble only from a strong base — a whiff overswings (−15 气)
          const bare = tgts.find(x => x.armorH <= 15);
          if (bare && hitBreakdown(u, bare).chance >= 50) { await doAttack(u, bare, true); continue; }
        }
      }
    ...
    if (CAMPAIGN) {
      ...
      const verdict = { scenario: scenario.id, winner: e === 0 ? "player" : "enemy",
        wear: units.filter((u) => u.side === "player").map((u) => ({
          ...
          wpns: [u.wpn, u.wpn2].filter(Boolean).map((w) => (
            { base: w.label.split("·").pop(), dura: w.duraNow })),
        })) };
      try {
...
function renderSkillbar() {
  ...
  let html =
    `<button class="skill ${selectedSkill === "attack" ? "sel" : ""}" onclick="uiSelectSkill('attack')" ${canAttack(u) ? "" : "disabled"}>${VERB[w.label.split("·").pop()] || "攻击"}<span>${w.ap}行动·${w.br}气</span></button>`;
  if (sp) {
...
function renderHover(u) {
  ...
  const critTxt = opts.forceHead ? `必中头部 ×${opts.headMult || (act.wpn.chop ? 2.25 : 1.5)}`
    : act.wpn.noHead ? "不能爆头"
...
function renderInspect(u) {
  ...
  if (w.special) {
    const SPDESC = {
      ...
      demolish: "破甲重击：甲伤 ×3，余势透甲伤血（不吃爆头加成）",
      aimed: "准头 +10，距离衰减减半",
      sweep: "命中所有相邻者——敌我不分，准头 −10",
      headhunt: "豪赌一鞭：必中头部（×2.0），但准头 −15——抡空泄气，气力额外 −15",
    };
    traits.push(`特技【${w.special.label}】${w.special.ap}行动力/${w.special.br}气力 — ${SPDESC[w.special.type]}`);
  }
  if (u.spearwall) traits.push("枪林立定中");
  if (u.wpn2) traits.push(`副械：${qSpan(u.wpn2.label, u.wpn2.quality)}（坠气 ${u.wpn2.br_tax}，换械 4 行动力）`);
  ...
  el.innerHTML =
    ...
    `血 ${u.hp}/${u.hpMax} ｜ ${qSpan(u.armorName, u.armorQ)} ${u.armorB}/${u.armorB0} · ${qSpan(u.helmName, u.helmQ)} ${u.armorH}/${u.armorH0} ｜ 招架 ${u.def}${u.shield ? "+" + u.shield : ""}<br>` +
    `甲械坠气 ${u.carryTax}（气力上限 ${u.breathBase}−坠气＝${u.breathMax}）<br>` +
    `兵器 ${qSpan(w.label, w.quality)}：伤 ${w.dmin}–${w.dmax}${w.duraNow != null ? `（耐久 ${w.duraNow}/${w.dura}${w.duraNow <= 0 ? "·卷刃" : ""}）` : ""} ｜ 破甲 ${Math.round(w.armorEff * 100)}% ｜ 穿甲 ${Math.round(w.pierce * 100)}%` +
    ` ｜ 每击 ${w.ap} 行动力 / ${w.br} 气力 ｜ 坠气 ${w.br_tax}<br>` +
...
async function boot() {
  ...
  units = scenario.units.map(su => {
    ...
    seenIds.add(su.id);
    let cond = null;
    if (CGEAR && tplById[su.id].side === "player" && CGEAR[su.id]) {
      cond = CGEAR[su.id];
      su = Object.assign({}, su, cond);          // the smith's work rides to war
    }
    const un = mkUnit(tplById[su.id], su.spawn[0], su.spawn[1], su); // su carries 品阶 overrides
